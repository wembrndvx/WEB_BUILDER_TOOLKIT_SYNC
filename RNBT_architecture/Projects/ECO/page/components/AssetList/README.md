# AssetList

계층적 자산 트리와 테이블을 표시하는 패널 컴포넌트

## 기능

### 트리 패널 (좌측)
- **계층 트리 표시**: 건물 > 층 > 방 > 랙 > 장비 계층 구조
- **Lazy Loading**: 노드 펼침 시 하위 데이터 로딩
- **트리 검색**: 노드 이름으로 필터링
- **일괄 펼침/접기**: 전체 노드 펼침/접기 버튼

### 테이블 패널 (우측)
- **자산 목록 표시**: 선택한 노드 하위의 모든 자산
- **검색**: 자산 이름/ID로 텍스트 검색
- **필터**: 타입별(UPS/PDU/CRAC/Sensor 등), 상태별(Normal/Warning/Critical)
- **가상 스크롤**: Tabulator 기반 대용량 데이터 처리
- **행 클릭 → Modal**: 자산 타입별 API 호출 후 상세 정보 Modal 표시

### 다국어(i18n) 지원
- **locale 파라미터**: API 요청 시 locale 전달
- **번역 필드**: `typeLabel`, `statusLabel` 사용
- **locale 변경 이벤트**: GlobalDataPublisher 구독

## UI 구조

```
┌─────────────────────────────────────────────────────────────────┐
│  Asset List                                            [↻]      │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  ┌─────────────────┐ │ ┌───────────────────────────────────────┐│
│  │      Tree       │ │ │  본관 > 1층 > 서버실 A    (10개)       ││
│  │                 │ │ ├───────────────────────────────────────┤│
│  │  🏢 본관        │ │ │  [Search...]  [Type ▼] [Status ▼]     ││
│  │   └─ 📂 1층     │ │ ├───────────────────────────────────────┤│
│  │      └─ 🚪 서버실│ │ │  ID    Name      Type    Status       ││
│  │                 │ │ │  ────────────────────────────────────  ││
│  │  🚪 독립 전산실  │ │ │  ...   ...       서버    정상          ││
│  │                 │ │ │  ...   ...       UPS     경고          ││
│  └─────────────────┘ │ └───────────────────────────────────────┘│
│                                                                  │
├─────────────────────────────────────────────────────────────────┤
│  [● Normal] [● Warning] [● Critical]          Total: 45         │
└─────────────────────────────────────────────────────────────────┘
```

## API 연동

### Subscriptions

| Topic | Handler | 설명 |
|-------|---------|------|
| `hierarchy` | `renderTree` | 트리 데이터 렌더링 |
| `hierarchyAssets` | `renderTable` | 테이블 데이터 렌더링 |
| `hierarchyChildren` | `appendChildren` | Lazy Loading 데이터 추가 |
| `locale` | `setLocale` | locale 변경 처리 |

### 발행 이벤트

| Event | Payload | 설명 |
|-------|---------|------|
| `@hierarchyNodeSelected` | `{ nodeId, node, locale }` | 트리 노드 선택 |
| `@hierarchyChildrenRequested` | `{ nodeId, locale }` | Lazy Loading 요청 |
| `@localeChanged` | `{ locale }` | locale 변경 알림 |

### 행 클릭 → Modal 흐름

테이블 행 클릭 시 자산 타입에 따라 API를 호출하고 Modal로 상세 정보를 표시합니다:

```
행 클릭 → onRowClick(asset)
    │
    ├─→ 타입 확인 (ups/pdu/crac/sensor)
    │
    ├─→ fetchData(datasetName, { assetId, locale })
    │       예: fetchData('ups', { assetId: 'ups-001', locale: 'ko' })
    │
    └─→ showModal(type, detail)
            │
            └─→ API 응답의 fields 배열로 동적 렌더링
```

**지원 타입 및 API 매핑**:

| 타입 | Dataset | API |
|------|---------|-----|
| ups | ups | GET /api/ups/:id?locale=ko |
| pdu | pdu | GET /api/pdu/:id?locale=ko |
| crac | crac | GET /api/crac/:id?locale=ko |
| sensor | sensor | GET /api/sensor/:id?locale=ko |

### API 응답 구조

**hierarchy (트리용)**
```javascript
{
    response: {
        data: {
            items: [
                {
                    id: "building-001",
                    name: "본관",
                    type: "building",
                    typeLabel: "건물",
                    status: "warning",
                    statusLabel: "경고",
                    canHaveChildren: true,
                    hasChildren: true,
                    children: [...]
                }
            ],
            summary: { depth: 1 }
        },
        meta: { locale: "ko" }
    }
}
```

**hierarchyAssets (테이블용)**
```javascript
{
    response: {
        data: {
            nodeId: "room-001-01-01",
            nodeName: "서버실 A",
            nodePath: "본관 > 1층 > 서버실 A",
            nodeType: "room",
            nodeTypeLabel: "방",
            assets: [
                {
                    id: "server-001",
                    name: "Server 001",
                    type: "server",
                    typeLabel: "서버",
                    status: "normal",
                    statusLabel: "정상"
                }
            ],
            summary: { total: 10, byStatus: {...} }
        },
        meta: { locale: "ko" }
    }
}
```

## Public Methods

| 메서드 | 파라미터 | 설명 |
|--------|----------|------|
| `renderTree` | `{ response }` | 트리 데이터 렌더링 |
| `renderTable` | `{ response }` | 테이블 데이터 렌더링 |
| `appendChildren` | `{ response }` | Lazy Loading 데이터 추가 |
| `toggleNode` | `nodeId` | 노드 펼침/접기 |
| `selectNode` | `nodeId` | 노드 선택 |
| `expandAll` | - | 전체 펼침 |
| `collapseAll` | - | 전체 접기 |
| `search` | `term` | 테이블 검색 |
| `filterByType` | `type` | 타입 필터 |
| `filterByStatus` | `status` | 상태 필터 |
| `setLocale` | `{ response }` | locale 설정 |

## 데이터 케이스

| 케이스 | 설명 | 예시 |
|--------|------|------|
| 케이스 1 | 일반 계층 | 본관 > 1층 > 서버실 > Rack > Server |
| 케이스 2 | 컨테이너 PDU | 전산실 > PDU Main > Circuit |
| 케이스 3 | 빈 컨테이너 | 별관 B > 2층 (하위 없음) |
| 케이스 4 | 독립 공간 | 외부 창고 (Building 없이 root-level Room) |

## 파일 구조

```
AssetList/
├── i18n/
│   ├── ko.json           # 한국어 UI 텍스트
│   ├── en.json           # 영어 UI 텍스트
│   └── ja.json           # 일본어 UI 텍스트
├── scripts/
│   ├── register.js       # 메인 로직 (트리/테이블/이벤트)
│   └── beforeDestroy.js  # 정리 (구독 해제, 테이블 파괴)
├── styles/
│   └── component.css     # 다크 테마 스타일
├── views/
│   └── component.html    # UI 구조
├── preview.html          # 독립 실행 테스트 (Mock Server 필요)
├── codeflow.md           # 코드 흐름 설명
└── README.md
```

## UI 다국어 (i18n)

UI 텍스트는 클라이언트 JSON 파일로 관리됩니다.

### JSON 구조

```json
{
    "panel": { "title": "자산 목록" },
    "tree": { "title": "계층 구조", "searchPlaceholder": "...", ... },
    "table": { "columns": { "id": "ID", "name": "이름", ... }, ... },
    "filter": { "allTypes": "전체 타입", ... },
    "status": { "normal": "정상", "warning": "경고", "critical": "위험" },
    "footer": { "total": "전체:", "count": "{count}개" },
    "actions": { "refresh": "새로고침" }
}
```

### 다국어 분리

| 구분 | 관리 주체 | 설명 |
|------|----------|------|
| UI 텍스트 | 클라이언트 (i18n/*.json) | 패널 제목, 버튼, placeholder 등 |
| 자산 데이터 | 서버 (locale 파라미터) | 자산 이름, typeLabel, statusLabel |

## preview.html 테스트

```bash
# 1. Mock Server 실행
cd ECO/mock_server
npm start  # http://localhost:4004

# 2. preview.html을 브라우저에서 열기
# (Live Server 등 사용)
```

## 참고 문서

- [API_SPEC.md](../../API_SPEC.md) - API 명세
- [I18N_SPEC.md](../../I18N_SPEC.md) - 다국어 설계 명세
- [codeflow.md](codeflow.md) - 코드 흐름 설명

---

*최종 업데이트: 2026-01-14*
